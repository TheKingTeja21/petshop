...
module.exports = {
  ...
  product: async (req, res) => {
    ...
    try {
      ...
      if(!product){
        return res.status(404).json("Not found Product")&& console.log(product);
  ...
  UpdateFoodId: async (req,res)=>{
    const productId = req.params.id;
    try {
      const updateProductId = await Product.findByIdAndUpdate(productId,req.body,{new:true,runValidators:true}); 
      if(!updateProductId){
        return res.status(404).json("product not found");
      }
      await Product.save()
  ...
  addProductType: async (req,res)=>{
    ...
    try {
      const product = await Product.findById(productId)
      if(!product) {
        return res.status(404).json({staus:false,message:'not found',error})
      }
      if(Product.ProductType.includes(productType)){
        return res.status(404).json({staus:false,message:'tag already exists'});
      }
